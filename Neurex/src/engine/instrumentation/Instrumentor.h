	...
	void BeginSession(const std::string& name, const std::string& filepath = "results.json")
	{
		...
		if (m_CurrentSession) {
			...
			if (Logger::get_core_logger()) // Edge case: BeginSession() might be
										   // before Log::Init()
			{
				NX_CORE_ERROR("Instrumentor::BeginSession('{0}') when session "
							  "'{1}' already open.",
...
#define NX_PROFILE_BEGIN_SESSION(name, filepath) ::Engine::Instrumentor::Get().BeginSession(name, filepath)
#define NX_PROFILE_END_SESSION() ::Engine::Instrumentor::Get().EndSession()
#define NX_PROFILE_SCOPE_LINE2(name, line)                                                                        \
	constexpr auto fixedName##line = ::Engine::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");          \
