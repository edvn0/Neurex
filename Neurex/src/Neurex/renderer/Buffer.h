...
namespace Neurex {
	...
	struct BufferElement
	{
		...
		BufferElement(ShaderDataType type_, const std::string& name_, bool normalize = false) : 
			name(name_), type(type_), size(shader_data_type_size(type_)), offset(0), normalization(normalize)
		{
		};
	...
	public:
		BufferLayout() {};
		BufferLayout(const std::initializer_list<BufferElement>& elmns) : elements(elmns) 
		{
