...
void OrthographicCameraController::on_event(Event& e)
{
	...
	dispatcher.dispatch_event<MouseButtonPressedEvent>([&](MouseButtonPressedEvent& ev) {
	...
	dispatcher.dispatch_event<MouseButtonReleasedEvent>([&](MouseButtonReleasedEvent& ev) {
		if (ev.get_mouse_button() != NX_MC_BUTTON_LEFT)
			return false;
		...
		if (is_pressed) {
			...
			camera_position += glm::vec3(-diff_x / 1200.0f, diff_y / 800.0f, 0.0f);
			camera.set_position(camera_position);

			NX_INFO("Vector difference cursor and chosen: {0}, {1}", diff_x, diff_y);
			is_pressed = false;
		}
		return false;
	});
